...
~25k non-enzymes overwhelm ~200–2000 enzyme samples per class.
...
class EnzymeClassifier:
    ...
    def _prepare_stages(self):
        ...
        n_class0_target = min(n_enzyme * 3, int((self.y_train_s1 == 0).sum()))  # at most 3:1 ratio
        ...
        self.y_train_s2 = self.y_train[enzyme_mask]
    ...
    def search_hyperparameters(self, stage, param_distributions, cv_folds=3,
                                scoring="f1_macro", verbose=1):
        """
        Run hyperparameter search for the specified stage (1 or 2).
        """
        self._check_loaded()
        ...
        base_params = {
            "objective": objective,
            "eval_metric": "logloss" if stage == 1 else "mlogloss",
            "tree_method": "hist",
            "subsample": 0.8,
    ...
    def predict(self, X):
        ...
        # Samples predicted as enzyme → stage 2
        enzyme_mask = s1_pred == 1
        if enzyme_mask.any():
            X_enzyme = X[enzyme_mask] if hasattr(X, 'iloc') else X[enzyme_mask]
            s2_probs = self.model_s2.predict_proba(X_enzyme)    # shape (m, 6): P(EC1)..P(EC6)
            s2_pred = self.model_s2.predict(X_enzyme)           # labels 1–6
    ...
    def feature_importance_per_class(self, top_n=10):
        """Per-class importance for stage 2 (enzyme classifier)."""
    ...
    def save_model(self, path):
        ...
        s1_path = out.parent / f"{out.stem}_s1{out.suffix}"
        s2_path = out.parent / f"{out.stem}_s2{out.suffix}"
